...
class RDPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        token_len_diff: torch.FloatTensor,
        ...
        if self.loss_type == "sigmoid":
            losses = (
                -F.logsigmoid(self.beta * logits + self.alpha * token_len_diff) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits + self.alpha * token_len_diff) * self.label_smoothing
            )
    ...
    def concatenated_forward(
        ...
        # get len(chosen)-len(rejected) (in tokens)
        chosen_len = (loss_mask[:len_chosen, :] == True).sum(dim=1)    
        rejected_len = (loss_mask[len_chosen:, :] == True).sum(dim=1)
        token_len_diff = chosen_len - rejected_len
            
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, token_len_diff)
